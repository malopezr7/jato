...
describe("loadJato", () => {
  it("loads a minimal rig", async () => {
    ...
    expect(rig.providerDocs).toEqual({});
    expect(rig.skills).toEqual([]);
    expect(rig.agents).toEqual([]);
  });

  it("loads a full rig with all files", async () => {
    ...
    expect(rig.manifest.mcp_servers).toHaveLength(1);
    expect(rig.instructions).toContain("Be awesome");
    expect(rig.providerDocs["claude"]).toContain("Use tools");
    expect(rig.providerDocs["codex"]).toContain("Be concise");
    expect(rig.skills).toHaveLength(2);
    expect(rig.skills.map((s) => s.name).sort()).toEqual(["code-review", "testing"]);
    expect(rig.agents).toHaveLength(1);
    expect(rig.agents[0].name).toBe("reviewer");
  });
